...
def synthesize_brand_intel(url: str, brand_name: str, client: genai.Client) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
             ...
             text_content = soup.get_text(separator=' ', strip=True)[:3000] # Limit tokens
    except Exception as e:
        print(f"Warning: Failed to scrape {url}. Relying on brand name only. Error: {e}")
    ...
    try:
        ...
        return {"brand_voice": "professional and modern", "keywords": [brand_name], "hashtags": [f"#{brand_name.replace(' ', '')}"]}

def generate_social_content(inputs: dict, brand_intel: dict, market_intel: str, client: genai.Client) -> dict:
...
def generate_base_image(image_prompt: str, client: genai.Client) -> Path:
    ...
    try:
        ...
        print(f"Warning: Native Image generation failed or unauthenticated ({e}). Yielding a fallback placeholder.")
...
def run_pipeline(input_data: dict) -> dict:
    ...
    api_key = os.environ.get("GEMINI_API_KEY", "AIzaSyAa62phHoFfxpE0_SqUtmWxfFuDsLpBNO8")
    if not api_key:
         raise PipelineError("GEMINI_API_KEY environment variable not set. Please set it to run the AI engine.")
    
    client = genai.Client(api_key=api_key)
    
    print("[1] Extracting Brand Intelligence...")
    brand_intel = synthesize_brand_intel(input_data.get("website_url"), input_data.get("brand_name"), client)
    
    print("[1.5] Fetching Market Intelligence via Search Grounding...")
    try:
        from market_intel import get_market_intelligence
        intel_res = get_market_intelligence(input_data.get('topic'), input_data.get('brand_name'))
        market_intel_txt = intel_res.get('intelligence', '')
    except Exception as e:
        print(f"Market Intel Warning: {e}")
        market_intel_txt = ""
    
    print("[2] Generating Social Content...")
    content = generate_social_content(input_data, brand_intel, market_intel_txt, client)
    
    print("[3] Generating Base Media...")
    base_img_path = generate_base_image(content.get("image_prompt", "Corporate background image"), client)
